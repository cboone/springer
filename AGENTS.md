...
- **`lint-text.yml` cannot be used, so `text-lint.yml`'s first job is inlined.** At `cboone/gh-actions` v3.0.0 its tool-install step reads `github.job_workflow_sha`, which arrives empty, and the step is unconditional, so the job fails before any linter runs. Tracked as [cboone/gh-actions#83](https://github.com/cboone/gh-actions/issues/83), unfixed in any release. Measured 2026-09-08: that job failed exactly this way while `lint-shell.yml`, `lint-github-actions.yml` and `scan-for-secrets.yml`, called the same way in the same run, all passed. None of those reads the context value. Switch back to the reusable call when a release fixes it.
