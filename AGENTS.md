...
Everything that runs today is static analysis. All four tools are installed locally and all four run in CI.

```bash
typos                          # spell check; config in typos.toml
markdownlint-cli2 "**/*.md"    # prose; config in .markdownlint-cli2.jsonc
prettier --check .             # JSON and YAML only, never Markdown or docs/design/
