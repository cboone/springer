...
| Step | Work                                                                             | Status |
| ---- | -------------------------------------------------------------------------------- | ------ |
| 0.1  | Scaffold agent config, `CHANGELOG.md`, `.gitignore` merge, plans `.gitkeep`      | Done   |
| 0.2  | Secret scanning: `gitleaks.yml`, `trufflehog.yml`, `.gitleaks.toml`              | Done   |
| 0.3  | Lint configuration, including a hand-written `typos.toml`                        | Done   |
| 0.4  | `.github/workflows/text-lint.yml`                                                | Done   |
| 0.5  | `AGENTS.md` with real content, replacing the scaffold stub                       | Done   |
| 0.6  | `.github/*.instructions.md` and the `copilot-instructions.md` pointers           | Done   |
| 0.7  | Community files: CONTRIBUTING, code of conduct, security policy, PR template     | Done   |
| 0.8  | `docs/adr/README.md` and ADRs 0001 through 0013                                  | Done   |
| 0.9  | README rewritten; these outcomes recorded                                        | Done   |

Six things landed differently from how this plan scoped them, each deliberately:

- **`text-lint.yml`'s first job is inlined rather than a reusable call, because the reusable workflow is broken.** `lint-text.yml` at `cboone/gh-actions` v3.0.0 reads `github.job_workflow_sha` in its tool-install step, which arrives empty, and the step is unconditional, so the job fails before a linter runs. Found by running it: the first CI run on this branch failed exactly that way while `lint-shell.yml`, `lint-github-actions.yml` and `scan-for-secrets.yml`, called identically in the same run, all passed. It is already tracked upstream as [cboone/gh-actions#83](https://github.com/cboone/gh-actions/issues/83), open since 2026-08-28 and unfixed in any release, with `cboone/audio-tools` carrying the same workaround adopted here: inline the steps against a committed `package-lock.json`, which preserves the per-package integrity checking the reusable workflow was reaching for. `markdownlint-cli2` is pinned at 0.23.2 rather than the 0.21.0 installed locally, because 0.21.0 pulls `js-yaml` and `markdown-it` versions carrying published denial-of-service advisories.

- **No `ci.yml`.** The `set-up-ci` Zig template is a call to a reusable workflow with `zig-version-file: build.zig.zon`, and that file is a Phase 1 deliverable. Shipping it now would mean ending Phase 0 with red CI. `text-lint.yml`, `gitleaks.yml` and `trufflehog.yml` cover the Phase 0 gate; `ci.yml` arrives in Phase 1 alongside the file it reads.
- **`bootstrap-project` was not used as the orchestrator.** Its installed cache copy at v1.3.1 directs the agent to read command files that do not exist in the plugin repository and invokes the setup skills under names they no longer have. The individual skills were invoked directly, in that orchestrator's own documented order.
- **`typos.toml` was written by hand.** No skill offers typos outside its Rust path, and the Phase 0 gate names it explicitly. Both entries carry fosforo's reasoning: a backtick-anchored pattern for the short commit SHAs that appear throughout the prose, and a `spellchecker:off` / `spellchecker:on` span. The second stopped being hypothetical immediately, since the Phase 0 plan tripped it while describing it.
- **`Middle C Is` and `Trace Chord Names` are deferred rather than ported**, and the reason is recorded in [ADR 0013](../../adr/0013-defer-the-gui.md). Neither has a surface before the GUI, and stable `clap_id`s mean adding a parameter later disturbs no automation, which is exactly the constraint Scripter imposed and CLAP removes.
- **Prettier is scoped away from Markdown and from `docs/design/`.** The second is load-bearing: Prettier formats `.js`, and reformatting the preserved v4 script would invalidate the Phase 2 reference vectors, which are generated by running that exact file.

**Exit criteria, all met:** `typos` clean; `markdownlint-cli2 "**/*.md"` reports 0 errors across 27 files; `prettier --check .` clean; `actionlint` silent; `gitleaks detect` reports no leaks. Every relative link in the ADRs, AGENTS.md, the instructions files and the community files resolves to a file that exists.

Two gaps are stated rather than counted as coverage. The shell job passes by finding nothing, since Phase 0 adds no shell scripts. And nothing builds, because there is no Zig yet.

One negative control was run and passed: with `.prettierignore` in place the v4 script is skipped, and with `--ignore-path /dev/null` Prettier flags it and would rewrite it. The entry is verified rather than assumed.
